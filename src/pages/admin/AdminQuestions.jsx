...
export default function AdminQuestions() {
  ...
  // Get unique speakers from questions
    ...
    <AdminLayout>
            ...
            <tbody>
              {questions.length === 0 ? (
                ...
                questions.map((question) => (
                  <tr key={question.id}>
                    <td>
                      <strong>{question.speaker_name}</strong>
                    </td>
                    <td>
                    ...
                    <td>
                      ...
                      <small style={{ color: 'rgba(255,255,255,0.5)' }}>
                        {question.attendee_email}
                      </small>
                      <br />
                      <small style={{ color: 'rgba(255,255,255,0.4)', fontFamily: 'monospace' }}>
                        {question.ticket_id}
                      </small>
                    </td>
                    <td>
                      <span style={{
                        ...
                        fontSize: '11px',
                        fontWeight: '600',
                        background: question.ticket_type.includes('VIP') || question.ticket_type.includes('Business') 
                          ? 'rgba(245,166,35,0.2)' 
                          : 'rgba(255,255,255,0.1)',
                        color: question.ticket_type.includes('VIP') || question.ticket_type.includes('Business')
                          ? '#f5a623'
                    ...
                    <td>
                      <select
                        ...
                        style={{
                          padding: '6px 10px',
                          background: question.status === 'selected' ? 'rgba(45,184,75,0.15)' :
                                     question.status === 'answered' ? 'rgba(26,115,232,0.15)' :
                                     question.status === 'rejected' ? 'rgba(232,49,42,0.15)' :
                                     'rgba(245,166,35,0.15)',
                          border: '1px solid',
                          borderColor: question.status === 'selected' ? '#2db84b' :
                                      question.status === 'answered' ? '#1a73e8' :
                                      question.status === 'rejected' ? '#e8312a' :
                                      '#f5a623',
                          borderRadius: '6px',
                          color: question.status === 'selected' ? '#2db84b' :
                                question.status === 'answered' ? '#1a73e8' :
                                question.status === 'rejected' ? '#e8312a' :
                                '#f5a623',
                          fontSize: '12px',
      ...
      <div className="admin-stats-grid" style={{ marginTop: '32px' }}>
        ...
        <div className="admin-stat-card" style={{ borderColor: '#1a73e8' }}>
          <div className="admin-stat-value" style={{ color: '#1a73e8' }}>
